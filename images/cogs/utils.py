from main import client, prefix as pre, prefixes
import psutil
import discord
import datetime
import asyncio
...
class Utility(commands.Cog):
    ...
    @commands.command()
    async def json(self, ctx, *, text):
        """ Yes, put whatever you want here and you might get featured in r!uptime (please don't put anything innapropriate)! """
        try:
            self.read_json("jsontxt.json")
            self.write_json(text, "jsontext.json")
            await ctx.send("Done")
        except:
            await ctx.send("An error occoured")
    ...
    async def prefixs(self, ctx):
        em = discord.Embed(title = "Prefixes!", descripton = "", color = ctx.author.color)
        index = 1
        em.set_footer(text = "These are my prefixes!")
        for prefix in prefixes:
            index += 1
            em.description += f"**{index}.** {prefix}>\n"
